...
Se ha utilizado *prisma orm* como mecanismo de persistencia en *mongodb*. La creación de colecciones se hace directamente en *mongosh* con el comando `db.createCollection(<name collection>, <options>)` y se ha usado la propiedad *$jsonSchema* para dar forma a las colecciones creadas, es decir que con este mecanismo se especifica las propiedades obligatorias de cada documento insertado así como las restricciones necesarias en la lógica de negocio. Por ejemplo par especificar que la variable *precio* del modificador debe tomar valores numericos mayores o iguales que cero se realiza de la siguiente manera:
...
El valor de esta variable es la cadena de conexión a la base de datos 'taco-feliz' en mongodb atlas y este valor es *secreto*.

Es necesario contar con registro en *mongodb atlas* para poder crear ahí la base de datos. Sin embargo puede probar la aplicación desplegada en render *https://taco-feliz.onrender.com*
